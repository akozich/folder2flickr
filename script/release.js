...
const token = process.env.FOLDER_2_FLICKR_GITHUB_TOKEN
...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'folder2flickr-mac.zip',
    path: path.join(outPath, 'Folder2Flickr-darwin-x64', 'Folder2Flickr.app')
  }, {
    name: 'folder2flickr-windows.zip',
    path: path.join(outPath, 'Folder2Flickr-win32-ia32')
  }, {
    name: 'folder2flickr-linux.zip',
    path: path.join(outPath, 'Folder2Flickr-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'Folder2FlickrSetup.exe',
      path: path.join(outPath, 'windows-installer', 'Folder2FlickrSetup.exe')
    }, {
      name: `folder2flickr-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `folder2flickr-${version}-full.nupkg`)
    }])
  })
}
...
function createRelease (assets) {
  const options = {
    uri: 'https://api.github.com/repos/akozich/folder2flickr/releases',
    headers: {
